...
public class RedisUtil {
	...
	public static void setHashValue(String key,Object key2,Object obj,int time){
		 redisTemplate.opsForHash().put(key,key2,obj);
	}
...
//     * @return
//     */
//    public static Long getNotPayOrder(String merchNo, String orderNo) {
//    	Long timeLive = redisNotifyTemplate.opsForValue().getOperations().getExpire(RedisConstants.cache_keyevent_not_pay_ord + merchNo + RedisConstants.link_symbol +  orderNo);
//    	return timeLive;
//    }
//
//
...
//				logger.info("订单异步回调通知时间：{}，{}，{}", merchNo,orderNo,minute);
//				if(minute == null){
...
//					
//					//System.out.println(">>>"+ RedisConstants.evtMinuteMap.get(minute));
...
//				if(minute == null){
//					minute = RedisConstants.keyevent_order_time;
//				}else if(minute == 0){
//					redisNotifyTemplate.opsForHash().delete(orderKey + merchNo, orderNo);
//					return false;
//				}
...
//		redisNotifyTemplate.delete(key);
//	}
//	
//	/**
//	 * @Description 获取商户余额
...
//
	  ...
	  public static void syncDict(DictDO dictDO, boolean delateFlag) {
	        if (dictDO == null) {
	            return;
	        }
	        if (ParamUtil.isNotEmpty(dictDO.getType())) {
	            if (delateFlag) {
	                redisTemplate.boundHashOps(RedisConstants.cache_dict_parent + dictDO.getType()).delete(dictDO.getName());
	            } else {
	                redisTemplate.boundHashOps(RedisConstants.cache_dict_parent + dictDO.getType()).put(dictDO.getName(), dictDO);
	            }
	        }
	  }
...
//     * @param index
//     */
//    public static void setHdPollingIndex(String merchNo,Integer index) {
//		redisTemplate.opsForHash().put(RedisConstants.hd_polling_index ,merchNo, index);
//	}
//    /**
//     * 取轮询index,为值则设置为0
//     * @param merchNo
//     * @param index
//     */
//    public static Integer getHdPollingIndex(String merchNo) {
//		Integer index=(Integer)redisTemplate.opsForHash().get(RedisConstants.hd_polling_index ,merchNo);
//		if(index==null) {
//			index=0;
//			setHdPollingIndex(merchNo,index);
//		}
//		return index;
//	}
//    
...
//     * @param orderNo
//     */
//    public static void setHdOrder(String hdOrderNo,String merchNo,String orderNo) {
//        redisTemplate.opsForHash().put(RedisConstants.cache_order_hd,hdOrderNo, merchNo+"|"+orderNo);
//      
//    }
//
//    public static String getHdOrder(String hdOrderNo) {
//        return (String) redisTemplate.opsForHash().get(RedisConstants.cache_order_hd, hdOrderNo);
//    }
//    
//    public static void removeHdOrder(String hdOrderNo) {
//        redisTemplate.opsForHash().delete(RedisConstants.cache_order_hd, hdOrderNo);
//    }
//    
//
//    
//    public static void setHdLockOrder(String lockOrderId,Order order, long minute) {
//    	redisNotifyTemplate.opsForValue().set(RedisConstants.cache_order_hd_lock + lockOrderId , order, minute, TimeUnit.MINUTES);
//    }
//    
//    public static Order getHdLockOrder(String lockOrderId) {
//    	return (Order)redisNotifyTemplate.opsForValue().get(RedisConstants.cache_order_hd_lock + lockOrderId );
//    }
//    
     ...
     * @param delateFlag
     */
    public static void syncPartnership(PartnershipDO partnershipDO, boolean delateFlag) {
        if (partnershipDO == null) {
            return;
        }
        if (ParamUtil.isNotEmpty(partnershipDO.getId())) {
            if (delateFlag) {
                redisTemplate.boundHashOps(RedisConstants.ad_partnership).delete(partnershipDO.getId());
            } else {
                redisTemplate.boundHashOps(RedisConstants.ad_partnership).put(partnershipDO.getId(), partnershipDO);
            }
        }
    }
    
    public static PartnershipDO getPartnership(Long key){
    	PartnershipDO partnershipDO=(PartnershipDO) redisTemplate.opsForHash().get(RedisConstants.ad_partnership, key);
    	return partnershipDO;
	}
    
    public static List<PartnershipDO> getPartnershipList(){
    	List<Object> list= redisTemplate.opsForHash().values(RedisConstants.ad_partnership);
    	List<PartnershipDO> tmepList=	BeanMapperUtil.mapList(list, PartnershipDO.class);
    	return tmepList;
	}
    
    public static Map<Long,PartnershipDO> getPartnershipMap(){
    	Map<Long,PartnershipDO> map=new HashMap<>();
    	List<Object> list= redisTemplate.opsForHash().values(RedisConstants.ad_partnership);
    	List<PartnershipDO> tmepList=	BeanMapperUtil.mapList(list, PartnershipDO.class);
    	for (PartnershipDO partnershipDO : tmepList) {
    		map.put(partnershipDO.getId(), partnershipDO);
		}
    	return map;
	}
     ...
     * @param delateFlag
     */
    public static void syncMarket(MarketDO marketDO, boolean delateFlag) {
        if (marketDO == null) {
            return;
        }
        if (ParamUtil.isNotEmpty(marketDO.getId())) {
            if (delateFlag) {
                redisTemplate.boundHashOps(RedisConstants.ad_market).delete(marketDO.getId());
            } else {
                redisTemplate.boundHashOps(RedisConstants.ad_market).put(marketDO.getId(), marketDO);
            }
        }
    }
    public static List<MarketDO> getMarketList(){
    	List<Object> list= redisTemplate.opsForHash().values(RedisConstants.ad_market);
    	List<MarketDO> tmepList=	BeanMapperUtil.mapList(list, MarketDO.class);
    	return tmepList;
	}
    
    public static Map<Long,MarketDO> getMarketMap(){
    	Map<Long,MarketDO> map=new HashMap();
    	List<Object> list= redisTemplate.opsForHash().values(RedisConstants.ad_market);
    	List<MarketDO> tmepList=	BeanMapperUtil.mapList(list, MarketDO.class);
    	for (MarketDO marketDO : tmepList) {
    		map.put(marketDO.getId(), marketDO);
		}
    	return map;
	}
     ...
     * @param delateFlag
     */
    public static void syncAdvertisement(AdvertisementDO advertisementDO, boolean delateFlag) {
        if (advertisementDO == null) {
            return;
        }
        if (ParamUtil.isNotEmpty(advertisementDO.getId())) {
            if (delateFlag) {
                redisTemplate.boundHashOps(RedisConstants.ad_advertisement).delete(advertisementDO.getId());
            } else {
                redisTemplate.boundHashOps(RedisConstants.ad_advertisement).put(advertisementDO.getId(), advertisementDO);
            }
        }
    }
    public static List<AdvertisementDO> getadvertisementList(){
    	List<Object> list= redisTemplate.opsForHash().values(RedisConstants.ad_advertisement);
    	List<AdvertisementDO> tmepList=	BeanMapperUtil.mapList(list, AdvertisementDO.class);
    	return tmepList;
	}
    
    
    public static AdvertisementDO getadvertisement(Long key){
    	AdvertisementDO advertisementDO=(AdvertisementDO) redisTemplate.opsForHash().get(RedisConstants.ad_advertisement,key);
    	return advertisementDO;
	}
    
    
    
    public static Map<Long,AdvertisementDO> getadvertisementMap(){
    	Map<Long, AdvertisementDO> map=new HashMap<>();
    	List<Object> list= redisTemplate.opsForHash().values(RedisConstants.ad_advertisement);
    	List<AdvertisementDO> tmepList=	BeanMapperUtil.mapList(list, AdvertisementDO.class);
    	for (AdvertisementDO advertisementDO : tmepList) {
    		map.put(advertisementDO.getId(), advertisementDO);
		}
    	return map;
	}
     ...
     * @param delateFlag
     */
    public static void syncProduct(ProductDO productDO, boolean delateFlag) {
        if (productDO == null) {
            return;
        }
        if (ParamUtil.isNotEmpty(productDO.getId())) {
            if (delateFlag) {
                redisTemplate.boundHashOps(RedisConstants.ad_product).delete(productDO.getId());
            } else {
                redisTemplate.boundHashOps(RedisConstants.ad_product).put(productDO.getId(), productDO);
            }
        }
    }
    public static List<ProductDO> getaProductList(){
    	List<Object> list= redisTemplate.opsForHash().values(RedisConstants.ad_product);
    	List<ProductDO> tmepList=	BeanMapperUtil.mapList(list, ProductDO.class);
    	return tmepList;
	}
    
    public static ProductDO getaProduct(Long key){
    	ProductDO productDO=(ProductDO) redisTemplate.opsForHash().get(RedisConstants.ad_product, key);
    	return productDO;
	}
    
    
    public static Map<Long,ProductDO> getaProductMap(){
    	Map<Long, ProductDO> map=new HashMap<>();
    	List<Object> list= redisTemplate.opsForHash().values(RedisConstants.ad_product);
    	List<ProductDO> tmepList=	BeanMapperUtil.mapList(list, ProductDO.class);
    	for (ProductDO productDO : tmepList) {
    		map.put(productDO.getId(), productDO);
		}
    	return map;
	}
     ...
     * @param delateFlag
     */
    public static void syncProductAdvertisement(ProductAdvertisementDO productAdvertisementDO, boolean delateFlag) {
        if (productAdvertisementDO == null) {
            return;
        }
        if (ParamUtil.isNotEmpty(productAdvertisementDO.getProductId())) {
            if (delateFlag) {
              redisTemplate.boundHashOps(RedisConstants.ad_product+RedisConstants.link_symbol+productAdvertisementDO.getProductId()).delete(productAdvertisementDO.getAdvertisementId());
            } else {
                redisTemplate.boundHashOps(RedisConstants.ad_product+RedisConstants.link_symbol+productAdvertisementDO.getProductId()).put(productAdvertisementDO.getAdvertisementId(), productAdvertisementDO);
            }
        }
    }
    public static List<ProductAdvertisementDO> getProductAdvertisementList(Long productId){
    	List<Object> list= redisTemplate.opsForHash().values(RedisConstants.ad_product+RedisConstants.link_symbol+productId);
    	List<ProductAdvertisementDO> tmepList=	BeanMapperUtil.mapList(list, ProductAdvertisementDO.class);
    	return tmepList;
	}
    
    public static void delProductAdvertisementByProductId(Long productId) {
    	 redisTemplate.delete(RedisConstants.ad_product+RedisConstants.link_symbol+productId);
    }
    
    
    
    public static void syncUser(AppUserDO appUserDO, boolean delateFlag) {
        if (appUserDO == null) {
            return;
        }
        if (ParamUtil.isNotEmpty(appUserDO.getMobile())) {
            if (delateFlag) {
              redisTemplate.boundHashOps(RedisConstants.yunxing_user).delete(appUserDO.getMobile());
            } else {
                redisTemplate.boundHashOps(RedisConstants.yunxing_user).put(appUserDO.getMobile(), appUserDO);
            }
        }
    }
    
    public static UserVo getUser(String accId){
    	UserVo userVo=(UserVo) redisTemplate.opsForHash().get(RedisConstants.yunxing_user, accId);
    	return userVo;
	}
    
    

    public static void setMessage(int messageType,String mobile, String value) {
    	String key=RedisConstants.yunxing_message+RedisConstants.link_symbol+messageType+RedisConstants.link_symbol+mobile;
        redisTemplate.opsForValue().set(key, value);
        redisTemplate.expire(key, 10, TimeUnit.MINUTES);
    }
    

    public static String getMessage(int messageType,String mobile) {
    	String key=RedisConstants.yunxing_message+RedisConstants.link_symbol+messageType+RedisConstants.link_symbol+mobile;
