...
//		showFooter:true,
		...
		columns : [
            ...
            },
			{
				...
				title : '订单状态',
				formatter:function(value){
					var	oss=orderStates[value];
					if(value==1){//成功
						return '<span class="label label-primary">'+oss+'</span>';
					}else
						return '<span class="label label-danger">'+oss+'</span>';
				}
			},
            {
                field : 'clearState',
                ...
                title : '通知状态',
                formatter:function(value){
                	var	nfs=noticeStates[value];
					if(value==1){//成功
						return '<span class="label label-primary">'+nfs+'</span>';
					}else
						return '<span class="label label-warning">'+nfs+'</span>';
                }
            },{
				field : 'detail',
				title : '详情',
				formatter:function(value,row,index){
					var html =  orderDetail(row,index);
					return html;
				}
			},{
				field : 'operate',
				...
				formatter:function(value,row,index){
					var html = "";
					if(row.orderState != 1 ){
						html += ' <a class="btn btn-primary " href="#" mce_href="#" title="手动同步支付订单" onclick="syncOrder(\''
							+ row.outChannel + '\',\'' + row.merchNo + '\',\'' + row.orderNo
							+ '\')">同步</a> ';
					}else if(row.orderState == 1 && row.noticeState==0){
						html += ' <a class="btn btn-success " href="#" mce_href="#" title="通知" onclick="noticeOrder(\''
							+ row.outChannel + '\',\'' + row.merchNo + '\',\'' + row.orderNo
							+ '\')">通知</a> ';
					}
					return html;
...
//            clearState:$('#clearState').val(),
        ...
        success : function(r) {
            if (r.code == 0) {
                ...
                var clearAmount = parseFloat(data.amount-data.clearAmount).toFixed(2);
                $("#clearAmount").text(" "+clearAmount+" ");
                $("#realAmount").text(" "+parseFloat(data.amount-data.qhAmount-clearAmount).toFixed(2)+" ");
                var where = "";
...
function noticeOrder(outChannel,merchNo,orderNo){
	var content = '<form class="form-horizontal m-t">' +
	  '<div class="col-sm-12">' +
	  		'<input id="notify_url" name="notify_url" class="form-control" type="text" maxlength="1000" placeholder="请输入回调地址">' +
	  '</div>' +
...
function ajaxNoiceOrder(outChannel,merchNo,orderNo,businessNo,layerIndex){
	$.ajax({
		type : "get",
		url : "/pay/noticeOrder/" + outChannel + "/" + merchNo + "/" + orderNo,
		data : {businessNo : businessNo},
		async : true,
		error : function(request) {
			parent.layer.alert("Connection error");
